...
from src.sections.terrain_data.terrain_table import TerrainTable

class TerrainData(BaseStruct):
    @staticmethod
    def set_terrain_table_repeats(_, instance: TerrainData):
        Retriever.set_repeat(TerrainData._terrain_table_float_ptrs, instance, instance.num_terrain_tables) # type: ignore
        Retriever.set_repeat(TerrainData._terrain_pass_graphic_ptrs, instance, instance.num_terrain_tables) # type: ignore
        Retriever.set_repeat(TerrainData.terrain_tables, instance, instance.num_terrain_tables) # type: ignore

    @staticmethod
    def sync_terrain_table_repeats(_, instance: TerrainData):
        if (
    ...
    @staticmethod
    def set_num_terrains(_, instance: TerrainData):
        TerrainTable.num_terrains = instance.num_terrains

    @staticmethod
    def sync_num_terrains(_, instance: TerrainData):
        num_terrains = None
    ...
    num_terrains: int                       = Retriever(uint16,                                                                 default = 113, on_read = [set_num_terrains], on_write = [sync_num_terrains])
