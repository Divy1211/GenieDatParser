...
class MapInfo2(BaseStruct):
    ...
    num_lands: int                  = Retriever(uint32,       default = 0, on_read = [set_lands_repeat])
    _land_ptr: bytes                = Retriever(Bytes[4],     default = b"\x00" * 4)
    lands: list[MapLand]            = Retriever(MapLand,      default_factory = MapLand)
    ...
    terrains: list[MapTerrain]      = Retriever(MapTerrain,   default_factory = MapTerrain)

    num_units: int                  = Retriever(uint32,       default = 0, on_read = [set_units_repeat])
    _unit_ptr: bytes                = Retriever(Bytes[4],     default = b"\x00" * 4)
    units: list[MapUnit]            = Retriever(MapUnit,      default_factory = MapUnit)
    ...
    elevations: list[MapElevation]  = Retriever(MapElevation, default_factory = MapElevation)
    # @formatter:on
